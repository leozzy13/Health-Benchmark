...
def parse_json_response(text: str) -> dict[str, Any]:
    ...
    raise ValidationError(f"Invalid JSON: {detail}")
...
def _json_parse_candidates(text: str) -> list[str]:
    ...
    fence_pattern = re.compile(r"```(?:json|JSON)?\s*([\s\S]*?)\s*```")
    for match in fence_pattern.finditer(base):
        block = match.group(1).strip()
        if block:
            candidates.append(block)

    for source in list(candidates):
        extracted = _extract_first_balanced_json_object(source)
        if extracted:
